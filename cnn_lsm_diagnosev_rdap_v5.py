...
cache_path=os.path.join(os.getcwd(), 'cache','cns_lstm')
...
df,tranlate_dic=startimportdata(df,figure_path,model_path,data_path,"cnn_lstm",300)
...
def objective(trial):
    ...
    for fold, (train_index, val_index) in enumerate(split):
        ...
        lockfile = f"{cache_path}/fold_{fold}.lock" 

        try:
            # =========================
            # If cache exists -> load
            # =========================
            if (os.path.exists(X_file) and
                os.path.exists(y_file) and
                os.path.exists(l_file)):

                print(f"📦 Loading cached balanced data: fold {fold}")

                X_train = np.load(X_file, mmap_mode="r")
                y_train = np.load(y_file, mmap_mode="r")
                len_train = np.load(l_file, mmap_mode="r")

            # =========================
            # Else -> create with lock
            # =========================
            else:
                acquire_lock(lockfile)

                try:
                    # Another process may have created while waiting
                    if not (os.path.exists(X_file) and
                            os.path.exists(y_file) and
                            os.path.exists(l_file)):

                        print(f"🧠 Creating balanced data for fold {fold}")

                        X_train, y_train, len_train = balance_sequences_hybrid_tsmote(
                            X_train, y_train, len_train
                        )

                        np.save(X_file, X_train)
                        np.save(y_file, y_train)
                        np.save(l_file, len_train)

                        print(f"✅ Finished caching fold {fold}")
                ...
                print("❌ Error during oversampling / caching")
                raise e
