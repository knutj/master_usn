...
def ensure_float32(X):
    try:
        ...
        print("❌ Failed to convert X to float32")
        print("Reason:", e)
        # Investigate the bad values
        print("Example bad value:", X.flat[0])
        raise e
...
# %%
figure_path=os.path.join(os.getcwd(), 'figures','lsm_diagt')
if not os.path.exists(figure_path):
    os.makedirs(figure_path)

model_path=os.path.join(os.getcwd(), 'models', 'lsm_diagt')
if not os.path.exists(model_path):
    os.makedirs(model_path)

data_path=os.path.join(os.getcwd(), 'data', 'cnn_lsm_diagt')
if not os.path.exists(data_path):
    os.makedirs(data_path)

cache_path=os.path.join(os.getcwd(), 'cache','lstm')
...
df = pd.read_csv("data/cleaned_data.csv.gz",nrows=20000)
#df = pd.read_csv("data/cleaned_data.csv.gz")
print(df.columns)

df,tranlate_dic=startimportdata(df,figure_path,model_path,data_path,"lstm",300)
...
# Step 1: Add labels to x_df
x_df = df.copy()
#x_df['label'] = y['label'].values

# Step 2: Create subject-level labels (e.g., max or any label for that subject)
subject_labels = x_df.groupby('id')['label'].max().reset_index()

# Step 3: Extract subject IDs and corresponding labels
subject_ids = subject_labels['id'].values
subject_y = subject_labels['label'].values

# Step 4: Split by subject_id (group) and stratify by subject-level label
...
# X data
X_train_all_ = x_df[x_df['id'].isin(train_subjects)].drop(columns=['label']).reset_index(drop=True)
X_test_ = x_df[x_df['id'].isin(test_subjects)].drop(columns=['label']).reset_index(drop=True)

# y data
y_train_all_ = pd.DataFrame({
    'label': x_df[x_df['id'].isin(train_subjects)]['label'].reset_index(drop=True)
})

y_test_ = pd.DataFrame({
    'label': x_df[x_df['id'].isin(test_subjects)]['label'].reset_index(drop=True)
})
...
split = list(skf.split(X_train_all, y_train_all, groups=group_train))
def objective(trial):
    import torch
    import torch.nn as nn
    from collections import Counter
    ...
    for fold, (train_index, val_index) in enumerate(split):
        ...
        model = LSTMStack_s(input_size=input_size, hidden_size=hidden_dim, num_layers=num_layers,dropout_prob=dropout,num_classes=num_class).to(device)
        ...
        X_train, y_train, len_train=cache_fold_balanced(fold, cache_path,X_train, y_train, len_train,balance_sequences_hybrid_tsmote,)  # pass balance_sequences_hybrid_tsmote here)
...
def build_model(params):
    model = LSTMStack_s(input_size=num_features, hidden_size=params["hidden_dim"], num_layers=params["num_layers"],dropout_prob=params["dropout"],num_classes=num_class)
    return model
...
print(train_text)
print(train_text)
with open(os.path.join(data_path,"lstm_training.txt"),'w') as f:
    f.write(train_text)
# %%
...
with open(os.path.join(data_path,"lstm_test.txt"),'w') as f:
