...
def balance_sequences_hybrid_tsmote(
    ...
    if platform.system() == "Windows":
        max_class_count = max(class_counts.values())*0.1
        max_samples = min(max_samples, 2000)
    else:
...
def acquire_lock(lockfile, sleep=0.5):
    while True:
        try:
...
def cache_fold_balanced(
    fold, cache_path,
    X_train, y_train, len_train,
    balance_fn,  # pass balance_sequences_hybrid_tsmote here
):
    os.makedirs(cache_path, exist_ok=True)

    X_file = os.path.join(cache_path, f"fold_{fold}_X.npy")
    y_file = os.path.join(cache_path, f"fold_{fold}_y.npy")
    l_file = os.path.join(cache_path, f"fold_{fold}_len.npy")
    lockfile = os.path.join(cache_path, f"fold_{fold}.lock")

    print("---- CACHE DEBUG ----")
    print("cache_path:", os.path.abspath(cache_path))
    print("X_file:", os.path.abspath(X_file))
    print("exists X/y/len:", os.path.exists(X_file), os.path.exists(y_file), os.path.exists(l_file))
    print("exists lock:", os.path.exists(lockfile))
    print("---------------------")

    try:
        # If all three exist, just load
        if os.path.exists(X_file) and os.path.exists(y_file) and os.path.exists(l_file):
            print(f"📦 Loading cached fold {fold}")
            Xb = np.load(X_file, mmap_mode="r")
            yb = np.load(y_file, mmap_mode="r")
            lb = np.load(l_file, mmap_mode="r")
            return Xb, yb, lb

        print(f"🔒 Acquiring lock for fold {fold} ...")
        acquire_lock(lockfile)
        print(f"✅ Lock acquired: {lockfile}")

        try:
            # Re-check after lock (maybe other process already created them)
            if os.path.exists(X_file) and os.path.exists(y_file) and os.path.exists(l_file):
                print(f"📦 Cache appeared while waiting. Loading fold {fold}.")
                return (np.load(X_file, mmap_mode="r"),
                        np.load(y_file, mmap_mode="r"),
                        np.load(l_file, mmap_mode="r"))

            print(f"🧠 Creating balanced data for fold {fold}")
            Xb, yb, lb = balance_fn(X_train, y_train, len_train)
            ...
            print("✅ Saved. Verifying existence:", os.path.exists(X_file), os.path.exists(y_file), os.path.exists(l_file))

        finally:
            release_lock(lockfile)
            print(f"🔓 Lock released: {lockfile}")

        print(f"📦 Loading saved fold {fold}")
        return (np.load(X_file, mmap_mode="r"),
                np.load(y_file, mmap_mode="r"),
                np.load(l_file, mmap_mode="r"))

    except Exception as e:
        print("❌ CACHE FAILED with exception:")
        traceback.print_exc()
        # Important: if we crash, try to release lock so you don't get stuck
        release_lock(lockfile)
        raise
